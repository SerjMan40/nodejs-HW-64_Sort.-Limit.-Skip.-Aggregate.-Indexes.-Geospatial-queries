    ...
    if (Array.isArray(articles)) {
      const validArticles = articles.map((item) => {
        return {
          ...
          framework: item.framework || 'No framework',
      ...
      res
        .status(201)
        .send(`Articles created successfully with IDs: ${addedArticles.map((article) => article._id).join(', ')}`)
    } else {
      const newArticle = { title: articles.title, framework: item.framework, year: item.year }
